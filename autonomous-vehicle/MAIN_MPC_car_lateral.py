...
# time from 0 to 10 seconds, sample time (Ts=0.1 second)
...
for i in range(0, sim_length-1):
    ...
    ft = np.matmul(np.concatenate(
        (np.transpose(x_aug_t)[0][0:len(x_aug_t)], r), axis=0), Fdbt)
    du = -np.matmul(np.linalg.inv(Hdb), np.transpose([ft]))
    x_aug_opt = np.matmul(Cdb, du)+np.matmul(Adc, x_aug_t)
    psi_opt = np.matmul(
        C_psi_opt[0:hz, 0:(len(states)+np.size(U1))*hz], x_aug_opt)
...
ac.vehicle_animation(t, statesTotal[:, 3], statesTotal[:, 1],
                     UTotal, X_ref, Y_ref, psi_ref, frame_amount, lf, lr, lane_width)
