...
class SupportFilesCar:
    ...
    def __init__(self):
        ...
        self.constants = [m, Iz, Caf, Car, lf, lr, Ts, Q, S, R, outputs, hz, x_dot, r, f,
                          time_length, lane_width, PID_switch, Kp_yaw, Kd_yaw, Ki_yaw, Kp_Y, Kd_Y, Ki_Y, trajectory]
    ...
    def state_space(self):
        ...
        A = np.array([[A1, 0, A2, 0], [0, 0, 1, 0], [
                     A3, 0, A4, 0], [1, x_dot, 0, 0]])
    ...
    def mpc_simplification(self, Ad, Bd, Cd, Dd, hz):
        ...
        C_aug = np.concatenate(
            (Cd, np.zeros((np.size(Cd, 0), np.size(Bd, 1)))), axis=1)
        D_aug = Dd
        ...
        for i in range(0, hz):
            ...
            for j in range(0, hz):
                if j <= i:
                    Cdb[np.size(B_aug, 0)*i:np.size(B_aug, 0)*i+B_aug.shape[0], np.size(B_aug, 1)*j:np.size(
                        B_aug, 1)*j+B_aug.shape[1]] = np.matmul(np.linalg.matrix_power(A_aug, ((i+1)-(j+1))), B_aug)
    ...
    def open_loop_new_states(self, states, U1):
        ...
        for i in range(0, sub_loop):
            ...
            y_dot_dot = -(2*Caf+2*Car)/(m*x_dot)*y_dot+(-x_dot -
                                                        (2*Caf*lf-2*Car*lr)/(m*x_dot))*psi_dot+2*Caf/m*U1
            psi_dot = psi_dot
            psi_dot_dot = -(2*lf*Caf-2*lr*Car)/(Iz*x_dot)*y_dot - \
                (2*lf**2*Caf+2*lr**2*Car)/(Iz*x_dot)*psi_dot+2*lf*Caf/Iz*U1
